...
def load_questions(question_file: str) -> dict:
    """
    加载问题文件，返回 {db_id: [questions]} 的映射
    支持 JSON 格式（Spider dev.json 格式）
    """
    questions_map = {}
    ...
    if question_file and os.path.exists(question_file):
        with open(question_file, 'r', encoding='utf-8') as f:
            ...
            for item in data:
                ...
                if db_id not in questions_map:
                    questions_map[db_id] = []
                questions_map[db_id].append(question)
    
    return questions_map


def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint, question_file=None):
    """
    修改后的评估函数，支持打印问题文本
    :param question_file: 可选的问题文件路径（JSON格式，包含 question 和 db_id 字段）
    """
    ...
    questions_list = []
    if question_file:
        questions_map = load_questions(question_file)
        # 也按顺序加载为列表
        if os.path.exists(question_file):
            with open(question_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
                questions_list = [item.get('question', '') for item in data]
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        ...
        for idx, pg in enumerate(zip(p, g)):
            ...
            # 获取当前问题文本
            current_question = ""
            if questions_list and global_question_idx < len(questions_list):
                current_question = questions_list[global_question_idx]
            ...
            if etype in ["all", "exec"]:
                ...
                if exec_score:
                    ...
                    # ===== 新增：执行错误时打印详细信息 =====
                    print("\n" + "="*80)
                    print(f"[EXEC ERROR] Index: {global_question_idx}, Hardness: {hardness}, DB: {db_name}")
                    if current_question:
                        print(f"Question: {current_question}")
                    print(f"Gold SQL: {g_str}")
                    print(f"Pred SQL: {p_str}")
                    print("="*80 + "\n")
            ...
            if etype in ["all", "match"]:
                ...
                if exact_score == 0:
                    ...
                    # ===== 修改：匹配错误时打印详细信息（包含问题） =====
                    print("\n" + "="*80)
                    print(f"[MATCH ERROR] Index: {global_question_idx}, Hardness: {hardness}, DB: {db_name}")
                    if current_question:
                        print(f"Question: {current_question}")
                    print(f"Gold SQL: {g_str}")
                    print(f"Pred SQL: {p_str}")
                ...
                entries.append({
                    ...
                    'question': current_question,  # 新增：保存问题到 entries
                    'db_id': db_name
                })
    ...
    return entries  # 返回 entries 以便进一步分析
...
if __name__ == "__main__":
    ...
    # ===== 新增参数：问题文件 =====
    parser.add_argument('--questions', dest='questions', type=str, default=None,
                        help='the path to the questions JSON file (e.g., dev.json) containing question and db_id fields')
    ...
    evaluate(args.gold, args.pred, args.db, args.etype, kmaps, args.plug_value, args.keep_distinct, 
             args.progress_bar_for_each_datapoint, question_file=args.questions)
